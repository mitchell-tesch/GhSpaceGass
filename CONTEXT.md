...
Core: `PreviewScaleHelper` (reusable: `ComputeAutoScale`, `ComputeBboxDiagonal`), `ReactionPreviewBuilder.Build` (pure data → `PreviewArrowResult` with `List<PreviewArrow>` + `ForceScale` + `MomentScale`), `PreviewArrow` model (Origin, Dx/Dy/Dz, Magnitude, ArrowType, Axis), `ArrowType` enum (Force/Moment). All existing inputs, outputs, data trees, filters, warnings, and status unchanged. 684 passing unit tests total (21 new).
